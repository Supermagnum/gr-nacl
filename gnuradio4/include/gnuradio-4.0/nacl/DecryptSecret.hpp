...
#ifndef GNURADIO4_NACL_DECRYPTSECRET_HPP
#define GNURADIO4_NACL_DECRYPTSECRET_HPP
...
GR_REGISTER_BLOCK(gnuradio4::nacl::DecryptSecret)

struct DecryptSecret : gr::Block<DecryptSecret, gr::NoTagPropagation> {
    using Description = gr::Doc<"Decrypts msg_encrypted tensors with crypto_secretbox_open_easy using a key from key_file_path.">;

    gr::MsgPortIn              msg_cipher_in{};
    gr::MsgPortOut             msg_plain_out{};
    gr::Annotated<std::string, "key_file_path"> key_file_path{};

    GR_MAKE_REFLECTABLE(DecryptSecret, msg_cipher_in, msg_plain_out, key_file_path);
    ...
    void processMessages(gr::MsgPortIn& port, std::span<const gr::Message> messages) {
        ...
        for (const gr::Message& message : messages) {
            ...
            if (cipherTensor == nullptr || nonceTensor == nullptr || cipherTensor->size() <= crypto_secretbox_MACBYTES //
                || nonceTensor->size() != crypto_secretbox_NONCEBYTES) {
                continue;
            }
            const std::size_t          plainLen = cipherTensor->size() - crypto_secretbox_MACBYTES;
            std::vector<unsigned char> plain(plainLen);
            const int                  err = ::crypto_secretbox_open_easy(plain.data(),
                cipherTensor->data(),
